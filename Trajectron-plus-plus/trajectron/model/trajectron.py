...
def sample_ego_states(ego_state, vel_scale=1.0, dt=0.5):
    ...
    for t in range(4, 0, -1):
        ...
        # delta_a = np.random.normal(0, vel_scale*np.linalg.norm(ego_state[t, 4:6]))  # 10% standard deviation
        # delta_yaw = np.random.normal(0, vel_scale*abs(ego_state[t, 6]))  # 10% standard deviation
...
class Trajectron(object):
    ...
    def run(self,
        ...
        for node_type in self.env.NodeType:
            ...
            # [41, 9, 8] [agent_idx, ts(t-8->t,his->cur), state]
            # {'position': ['x', 'y'], 'velocity': ['x', 'y'], 
            # 'acceleration': ['x', 'y'], 'heading': ['°', 'd°']}
            x = x_t.to(self.device)
            x_st_t = x_st_t.to(self.device)
            y = y_t.to(self.device)
            if robot_traj_st_t is not None:
    ...
    def eval(self,
        ...
        for node_type in self.env.NodeType:
            ...
            if det_cand is not None:
                ...
                # print(x[target_index][:,:2].shape)
                # print(torch.sum(x[target_index][-5:,:]))
                # raise ValueError()
            #### change target node: assign det ###
    ...
    def attack(self,
        ...
        for node_type in self.env.NodeType:
            ...
            if perturb_cand is not None:
                ...
                # perturb heading (range [-pi, pi])
                # # transform to [0, 2pi] to apply dh: if < 0, +2pi
                # x[target_index][-5:,-2:-1] = (x[target_index][-5:,-2:-1] + 2*np.pi) % (2*np.pi)
                # dh range in [-pi, pi]
    ...
    #             ego_states = x[ego_idx][-5:,:]  # torch size (5, 8)
    #             # print(x[ego_idx][-5:,:])
    #             ego_states_sampled = sample_ego_states(ego_states.detach().cpu().numpy(), vel_scale=ego_sample_scale)  # numpy size (1, 5, 8)
    #             x[ego_idx][-5:,:] = torch.from_numpy(ego_states_sampled).to(self.device)  # torch size (5, 8)
    #             # print(x[ego_idx][-5:,:])
    #             # raise ValueError()
    #         ### add perturbation to ego node ###
    ...
    #         # eval (nuScenes dataset 20hz)
    #         if eval_20hz:

    #             # if pos_20hz is None or heading_20hz is None:
    #             #     raise ValueError("heading_20hz or pos_20hz is None")
    #             # print(x[target_index][-5:,:2] + torch.tensor([scene.patch[0], scene.patch[1]]).cuda())
    #             # print(x[target_index][-5:,-2:-1])

    #             duration = perturbation["duration"]
    #             pos_20hz = perturbation["delta_position"]
    #             heading_20hz = perturbation["delta_heading"]

    #             # change position
    #             x[target_index][duration,:2] = pos_20hz[duration]
    #             x_st_t[target_index][-5:,:2] = x[target_index][-5:,:2] / 80  # standardize (find dev in process_data.py)
    ...
    #             # change heading (range [-pi, pi])
    #             x[target_index][duration,-2:-1] = heading_20hz[duration]
    #             x_st_t[target_index][-5:,-2:-1] = (x[target_index][-5:,-2:-1] - 0) / np.pi  # standardize to [-1, 1]
    ...
    #         # attack (apply perturbation)
    #         if attack and target_index >= 0:
                
    #             # print(x[target_index][-5:,:2] + torch.tensor([scene.patch[0], scene.patch[1]]).cuda())
    #             # print(x[target_index][-5:,-2:-1])

    #             duration = perturbation["duration"]
    #             dh = perturbation["delta_heading"]
    #             dp = perturbation["delta_position"]
                
    #             # random sample detection distribution, assign to first 4 frames
    #             if target_sample and target_dh_set is not None:
    #                 random_indices = torch.randperm(target_dh_set.shape[0])[:4]
    #                 random_samples = target_dh_set[random_indices]
    #                 dh[:-1, :] = random_samples

    #             # random sample detection distribution, assign to first 4 frames
    #             if target_sample and target_dp_set is not None:
    #                 # calc stat for dp
    #                 dx_mean, dx_std = torch.mean(target_dp_set[:, 0]), torch.std(target_dp_set[:, 0])
    #                 dy_mean, dy_std = torch.mean(target_dp_set[:, 1]), torch.std(target_dp_set[:, 1])
    ...
    #             # perturb position
    #             x[target_index][-5:,:2] += dp
    #             x_st_t[target_index][-5:,:2] += dp / 80  # standardize (find dev in process_data.py)
    ...
    #             # x[target_index][-5:,-2:-1] = (x[target_index][-5:,-2:-1] + 2*np.pi) % (2*np.pi)
    #             # dh range in [-pi, pi]
    #             x[target_index][-5:,-2:-1] += dh
    #             x[target_index][-5:,-2:-1] = (x[target_index][-5:,-2:-1] + np.pi) % (2*np.pi) - np.pi  # normalize to [-pi, pi]
    #             x_st_t[target_index][duration,-2:-1] = (x[target_index][duration,-2:-1] - 0) / np.pi  # standardize to [-1, 1]
