...
# for easy visualization in prediction attack (others, target)
...
def plot_attacked_vehicle_nice(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0,
                               vis_mode='all', target_instance_id=None):
    prediction_dict, histories_dict, futures_dict = prediction_output_to_trajectories(predictions,
                                                                                      dt,
                                                                                      max_hl,
                                                                                      ph,
    ...
    for node in node_list:
        ...
        # if node.id == 'ego' or node.id == target_instance_id:
...
def plot_attacked_vehicle_mm(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0, \
                    vis_mode='ego_adv', target_instance_id=None):
    # vis_mode: 'all', 'ego_w_target'
    #   - all: visualize all vehicles w bounding boxes
    #   - ego_adv: visualize only ego vehicle and adv vehicle wo bounding boxes
    ...
    node_list = sorted(histories_dict.keys(), key=lambda x: x.id)
    for node in node_list:
        history = histories_dict[node] + np.array([x_min, y_min])  # (1, 1, 6, 2)
        future = futures_dict[node] + np.array([x_min, y_min])

        predictions = prediction_dict[node] + np.array([x_min, y_min])  # (5, 2)
        

        if vis_mode == 'ego_adv':
            if node.id == 'ego':
                # plot vehicle
                r_img = rotate(robot, node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                            reshape=True)
                oi = OffsetImage(r_img, zoom=0.01, zorder=700)  # zoom: 0.025 -> 0.015
                veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                veh_box.zorder = 700
                ax.add_artist(veh_box)
                continue
                ...
                #     zorder=650, alpha=0.7)
            elif node.id == target_instance_id:
                # plot vehicle
                r_img = rotate(cars[-1], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                            reshape=True)
                oi = OffsetImage(r_img, zoom=0.01, zorder=700)  # zoom: 0.025 -> 0.015
                veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                veh_box.zorder = 700
            ...
            if node.id == 'ego':
                ...
                r_img = rotate(robot, node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                            reshape=True)
                oi = OffsetImage(r_img, zoom=0.015, zorder=700)  # zoom: 0.025 -> 0.015
                veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                veh_box.zorder = 700
                ax.add_artist(veh_box)
            elif node.id == target_instance_id:
                ...
                # plot vehicle
                r_img = rotate(cars[0], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                            reshape=True)
                oi = OffsetImage(r_img, zoom=0.015, zorder=700)  # zoom: 0.025 -> 0.015
                veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                veh_box.zorder = 700
