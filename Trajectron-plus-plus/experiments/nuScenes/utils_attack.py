...
def loss_matching(curr_perturb, cluster_perturb):
    # curr_perturb: shape (1, 3)
    # cluster_perturb: shape (1, 3)
    # L1 distance (use torch.norm)
    return torch.sum(torch.abs(curr_perturb - cluster_perturb))
    
    # error = torch.norm(curr_perturb - cluster_perturb, dim=-1)
    # return torch.mean(error, dim=-1)
    # L2 distance
    # return torch.sum(torch.square(curr_perturb - cluster_perturb))
### loss ###
