...
# def get_corners(center, dimensions, yaw):
#     """Get the corners of the rectangular bounding box."""
#     width, length = dimensions / 2
#     corners = np.array([
#         [length, width],
#         [-length, width],
#         [-length, -width],
...
#     """Check for collision between two bounding boxes using the Separating Axis Theorem."""
#     for a, b in zip(corners1, corners1[1:] + corners1[:1]):
#         axis = np.cross(b - a, np.array([0, 0, 1]))[:2]
#         projections1 = [np.dot(point, axis) for point in corners1]
#         projections2 = [np.dot(point, axis) for point in corners2]
#         if max(projections1) < min(projections2) or max(projections2) < min(projections1):
#             return False
#     return True

# def check_collision_bbox(center_tar, dim_tar, h_tar, center_vic, dim_vic, h_vic):
#     """Check for collision between two bounding boxes."""
#     # Rotate victim's bounding box into the target's coordinate system
#     relative_angle = h_tar - h_vic
    
#     for i in range(center_tar.shape[0]):
#         corners1 = get_corners(center_tar[i], dim_tar, h_tar)
#         rotated_center_vic = rotate(center_vic[i] - center_tar[i], -h_tar) + center_tar[i]
#         corners2 = get_corners(rotated_center_vic, dim_vic, relative_angle)
        
#         if separating_axis_theorem(corners1, corners2):
#             print(f"collision at frame {i+1}")
#             return True
...
#     for i in range(center_tar.shape[0]):
#         corners1 = get_corners(center_tar[i], dim_tar, h_tar)
#         corners2 = get_corners(center_vic[i], dim_vic, h_vic)
    
#         if separating_axis_theorem(corners1, corners2) and separating_axis_theorem(corners2, corners1):
#             print("collision at frame {}".format(i+1))
#             return True
    
#     return False

# v3: ellipse overlap
# generat a enlarged ellipse based on adv vehicle's bbox (consider ego vehicle's size)
...
def ekf_tracker(cand, scene_dt=0.5):
    ...
    # prev_x = target_prev[:, 0]
    # prev_y = target_prev[:, 1]
    # prev_heading = target_prev[:, 2]

    # x = np.concatenate((prev_x, x), axis=0)
    # y = np.concatenate((prev_y, y), axis=0)
    # heading = np.concatenate((prev_heading, heading), axis=0)
    ...
    # # Compute Curvature and Path Length
...
# def kalman_filter(x, y, heading, scene_dt, max_timesteps):
#     # Compute Velocity
#     vx = derivative_of(x, scene_dt)
#     vy = derivative_of(y, scene_dt)
#     velocity = np.linalg.norm(np.stack((vx, vy), axis=-1), axis=-1)
...
#     return x, y, heading, velocity  # Returns the updated values
# ### kalman filter ###
...
def output_to_csv_realworld(output_path, scene_type, scene_name, dataset, 
                            collision, pi_ade, pr_ade, lat_dev, long_dev, lat_jerk, long_jerk):
    ...
    if not os.path.exists(output_path):
        ...
        with open(output_path, 'w') as f:
            writer = csv.writer(f)
            header = ['timestep', 'scene_type', 'scene_name', 'dataset', 
                      'collision', 'piade', 'prade', 'lat_dev', 'long_dev', 'lat_jerk', 'long_jerk']
            writer.writerow(header)
    ...
    now = datetime.datetime.now()
    timestep = '{}/{} {}:{}'.format(str(now.month), str(now.day), str(now.hour), str(now.minute))
    row = [timestep, scene_type, scene_name, dataset, collision, pi_ade, pr_ade, lat_dev, long_dev, lat_jerk, long_jerk]

    # check if row with same scene type, name, and dataset exists
    existing_row = None
    for r in rows[1:]:  # skip header
        if r[1] == scene_type and r[2] == scene_name and r[3] == dataset:
            existing_row = r
            break
