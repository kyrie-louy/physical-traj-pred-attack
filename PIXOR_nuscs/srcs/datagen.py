...
# from attack_utils import trans_v2world
...
class KITTI(Dataset):
    ...
    # #defense strategy: randomly insert point cluster into original point cloud
    # def load_velo_scan_adv(self, item):
    #     filename = self.velo[item]
    #     gt3Dboxes = self.get_gt3Dboxes(item)
    #     c_name = bytes(filename, 'utf-8')
    #     scan = np.zeros(self.geometry['input_shape'], dtype=np.float32)
    #     c_data = ctypes.c_void_p(scan.ctypes.data)
    #     #print(item)
    #     N_add = 10
    #     xs = np.random.rand(N_add,1)*3.0-1.5
    #     ys = np.random.rand(N_add,1)*3.0-1.5
    #     zs = np.random.rand(N_add,1)*1.0+0.0
    #     rs = np.random.rand(N_add,1)*0.3+0.4
    #     #rs = np.ones((N_iter,N_add,1),dtype=np.float32)*0.5
    #     added_points = np.concatenate([xs,ys,zs,rs],axis=1)
    #     added_points = np.reshape(added_points, (N_add*4,))
    #     if len(gt3Dboxes)>=3:
    #         vehicle_idx = random.randrange(len(gt3Dboxes))
    #     else:
    #         vehicle_idx = 0
    #     #print(len(gt3Dboxes),vehicle_idx)
    #     if len(gt3Dboxes)==0:
    #         added_points_t = added_points
    #     else:
    #         added_points_t = trans_v2world(added_points,gt3Dboxes,vehicle_idx,N_add)
    #         #print(gt3Dboxes[vehicle_idx])
    #     add = np.asarray(added_points_t,dtype=np.float32)
    #     c_add = ctypes.c_void_p(add.ctypes.data) 
    #     ctypes.cdll.LoadLibrary('attack_preprocess/attackLidarPreprocess.so'). \
    #                 createTopViewMaps(c_data, c_name, c_add, ctypes.c_int(int(len(add)/4)))

    #     return scan

    # #defense strategy: insert point at given location of one vehicle (vehicle center)
    # def load_velo_scan_insert(self, item):
    #     filename = self.velo[item]
    #     gt3Dboxes = self.get_gt3Dboxes(item)
    #     c_name = bytes(filename, 'utf-8')
    #     scan = np.zeros(self.geometry['input_shape'], dtype=np.float32)
    ...
    #         added_points_t = trans_v2world(added_points,gt3Dboxes,vehicle_idx,N_add)
    #         #print(gt3Dboxes[vehicle_idx])
    #     add_all = np.asarray(added_points_t,dtype=np.float32)
    #     #add_all = np.empty((0,))
    #     #for vehicle_idx in range(len(gt3Dboxes)):
    #     #    added_points_t = trans_v2world(added_points,gt3Dboxes,vehicle_idx,N_add)
    #     #    add = np.asarray(added_points_t,dtype=np.float32)
    #     #    add_all = np.concatenate([add_all,add],axis=0)
    #     #
    #     #add_all = np.asarray(add_all,dtype=np.float32)
    #     c_add = ctypes.c_void_p(add_all.ctypes.data) 
    #     ctypes.cdll.LoadLibrary('attack_preprocess/attackLidarPreprocess.so'). \
    #                 createTopViewMaps(c_data, c_name, c_add, ctypes.c_int(int(len(add_all)/4)))
    #     if len(gt3Dboxes) != 0:
    #         yzx = np.array(gt3Dboxes,dtype=np.float32)[vehicle_idx,3:6]
    #     else:
    #         yzx = np.zeros((3,),dtype=np.float32)
    #     return scan,yzx
