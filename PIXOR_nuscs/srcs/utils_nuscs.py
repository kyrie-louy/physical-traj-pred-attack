...
# def get_gt_boxes3d_lidar(label_file, calib): 
...
def trans_v2world_realworld(addpts_center, gt3Dboxes, vehicle_idx, N_add, origin):
    # addpts_center: (N_add, 3)

    # check if no points
    if addpts_center.shape[0] == 0:
        return np.empty((0, ), dtype=np.float32)

    # read gt bbox in LiDAR coord
    w, h, l, y, z, x, yaw = gt3Dboxes[vehicle_idx]
    #print(w, h, l, y, z, x, yaw)
    
    # addpts_center offset to lidar
    # 1. select origin point 
    # 1.1 bottom top center
    board_trans = np.zeros((N_add, 3), dtype=np.float32)
    if origin == 'rear_left' or origin == 'front_left':
        board_trans[:, 0] = (x - l/2.) + (addpts_center[:,0] * np.cos(yaw) - addpts_center[:,1] * np.sin(yaw))
        board_trans[:, 1] = (y + w/2.) + (addpts_center[:,0] * np.sin(yaw) + addpts_center[:,1] * np.cos(yaw))
        board_trans[:, 2] = (z - h/2.) + addpts_center[:,2]
    elif origin == 'rear_right' or origin == 'front_right':
        board_trans[:, 0] = (x - l/2.) + (addpts_center[:,0] * np.cos(yaw) - addpts_center[:,1] * np.sin(yaw))
        board_trans[:, 1] = (y - w/2.) + (addpts_center[:,0] * np.sin(yaw) + addpts_center[:,1] * np.cos(yaw))
        board_trans[:, 2] = (z - h/2.) + addpts_center[:,2]
    else:
        raise ValueError('origin should be rear_left, front_left, rear_right, front_right')

    # 2. board center to corners
    N_board = N_add
    adv_bd_list = np.zeros((N_board, 6), dtype=np.float32)
    for i in range(N_board):
        adv_bd_list[i, 0] = board_trans[i, 0]  # x
        adv_bd_list[i, 1] = board_trans[i, 1]  # y
        adv_bd_list[i, 2] = board_trans[i, 2]  # z
        adv_bd_list[i, 3] = 0.42  # h
        adv_bd_list[i, 4] = 0.3  # w
        if origin == 'rear_left' or origin == 'front_left':
            adv_bd_list[i, 5] = -np.pi / 4  # yaw (positive rotate clockwise)
        elif origin == 'rear_right' or origin == 'front_right':
            adv_bd_list[i, 5] = np.pi / 4
        else:
            raise ValueError('origin should be rear_left, front_left, rear_right, front_right')
    
    adv_bd_corners = board_params2corners(adv_bd_list, N_board, rot='yaw')  # (N_board, 4, 3)

    # 3. corner raycast
    pts = np.empty((0, ), dtype=np.float32)
    for i in range(N_board):
        pts = np.concatenate([pts, cornersraycast(adv_bd_corners[i]).reshape(-1, )], axis=0)
